...
- [ ] **Phase 09.1: Strip-Only Mixer Rebuild On Stable Layout (INSERTED)** - Plan 01 is complete: the stable reverted page shell is preserved and the mixer now has a horizontal local-first strip plane with header-owned local controls and collapsed meter-only local mini strips; the narrower strip-widget conversion is next.
...
### Phase 08.3.4.1: Fixed-Bus Multi-Input And NINJAM Channel Implementation (INSERTED)
  
  **Goal:** Implement the first practical multi-I/O workflow validated by Phase 08.3.4: two local transmit paths via `Main` and `Local Send 2`, plus fixed extra output pairs for routed remote returns using host-owned Ableton routing.
  **Requirements**: ROUTE-02, ROUTE-03
**Depends on:** Phase 08.3.4
  **Success Criteria** (what must be TRUE):
  1. User can route a second Ableton track into `Local Send 2` and have FLJ transmit it as a separate NINJAM channel from the same plugin instance.
  ...
  **Plans:** 3/3 plans executed, functionally complete with deferred follow-on UX/persistence work
...
- [x] 08.3.4.1-03-PLAN.md - Rebuilt the fixed-bus artifact, validated the repaired Jim/Ben Ableton path, and closed the first-pass implementation with reopen persistence plus removable-channel UX deferred forward.

### Phase 08.3.4.2: Expanded Fixed-Bus I/O And Persistent Input Mapping (INSERTED)

**Goal:** Extend the current fixed-bus routing model beyond the first `Main` plus `Local Send 2` proof by targeting up to eight exposed inputs and outputs, while adding removable extra local channels and persistent input-side naming/routing.
**Depends on:** Phase 08.3.4.1
**Requirements:** ROUTE-04, ROUTE-05, ROUTE-06
**Success Criteria** (what must be TRUE):
1. Project proves what Ableton and VST3 will actually expose beyond the first extra sidechain input, with a practical ceiling documented up to an eight-input/eight-output target.
2. User can add and remove extra local channels from the FLJ UI instead of only adding them.
3. Input-side local channel identity persists across project restore: names, enabled slots, and bus-to-local-channel mapping are remembered when reopening a saved set.
4. Expanded remote output routing continues to use host-owned FLJ output bus pairs, with the user able to assign remote channels to the exposed fixed outputs that actually prove practical in Ableton.
5. The phase closes with an explicit implementation verdict on how many local inputs and remote outputs are realistically supportable in the current Ableton/VST3 path.
**Plans:** TBD
...
**Plans**: 4/4 plans executed, blocked by failed Ableton validation
...
- [x] 09-04-PLAN.md - Record the explicit Ableton verdicts for the Phase 09 layout matrix and name the residual host blocker. (completed 2026-04-07, blocked outcome)

### Phase 09.1: Strip-Only Mixer Rebuild On Stable Layout (INSERTED)
**Goal**: Preserve the stable reverted page layout and rebuild only the mixer-strip area around the approved HTML mockup, so FLJ gains a denser, Ableton-inspired strip workflow without rebreaking connection/session usability.
**Depends on**: Phase 9
**Requirements**: LAYOUT-01, LAYOUT-02, LAYOUT-03
**Success Criteria** (what must be TRUE):
1. The rest of the editor remains on the reverted pre-Phase-9 structure: connect/session workflow, sidebar, and footer do not regress while strip work lands.
2. Mixer strips move toward the approved narrow mockup: integrated long meter/fader spine, compact side controls, and clearly grouped local plus remote strip identity.
...
- [ ] 09.1-02-PLAN.md - Convert the strip shell toward the narrower integrated strip widget and compact side-control model.
- [ ] 09.1-03-PLAN.md - Polish strip density and record the focused Ableton strip-usability validation matrix.
...
| LAYOUT-01 | Phase 9 / Phase 09.1 |
| LAYOUT-02 | Phase 08.3 / Phase 9 / Phase 09.1 |
| LAYOUT-03 | Phase 08.2 / Phase 08.3 / Phase 08.3.1 / Phase 9 / Phase 09.1 |
| PAR-01 | Phase 10 |
| PAR-02 | Phase 10 |
...
**Execution Order:**
1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7 -> 8 -> 8.1 -> 8.2 -> 8.3 -> 8.3.1 -> 8.3.2 -> 8.3.3 -> 8.3.4 -> 8.3.4.1 -> 8.3.4.2 -> 9 -> 9.1 -> 10
...
| 9.1. Strip-Only Mixer Rebuild On Stable Layout | 1/3 | In Progress | - |
