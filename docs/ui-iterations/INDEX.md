...
- Current baseline commit before the next direct UI iteration: `79398c0`
...
| `UI-013` | in_progress | Shorten the expanded local group title from `Local Sends` to `Local` so it fits the narrow header more cleanly | `79398c0` | pending | `git revert <final-commit>` |

## Next ID

- Next feature entry: `UI-014`
